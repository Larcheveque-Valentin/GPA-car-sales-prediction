...
def preprocess_input(x):
    # Création d'une copie des données d'entrée pour les manipulations
    x_processed = x.copy()

    # Extraction du dixième caractère de la colonne 'SERIE' et création d'une nouvelle colonne 'dixieme_caractere'
    x_processed['dixieme_caractere'] = x_processed['SERIE'].str[9]
    ...
    x_processed['annee_construction'] = x_processed['dixieme_caractere'].map(correspondance_annee)
    ...
    date_columns_indices = [i for i, col in enumerate(x_processed.columns) if 'DATE' in col]
    # Conversion des colonnes de dates en objets de date
    for col_index in date_columns_indices:
        x_processed.iloc[:, col_index] = pd.to_datetime(x_processed.iloc[:, col_index])

    # Transformation de la colonne 'KILOMETRAGE' en valeurs numériques
    x_processed['KILOMETRAGE'] = pd.to_numeric(x_processed['KILOMETRAGE'], errors='coerce')

    # Calcul du nombre d'années écoulées depuis la date actuelle
    today_time = datetime.today()
    for col_index in date_columns_indices:
        x_processed.iloc[:, col_index] = (today_time - x_processed.iloc[:, col_index]).dt.days / 365.25

    # Conversion des colonnes en facteurs
    for column in ['CARBURATION', 'MARQUE', 'TYPE_DE_VEHICULE', 'DESTIN2', 'COULEUR', 'MODELE']:
        x_processed[column] = pd.factorize(x_processed[column])[0]

    # Création d'une copie des données sans la colonne "DESTIN2"
    X = x_processed.drop(["DESTIN2"], axis=1)

    # Conversion des données au format torch.Tensor
    X = torch.Tensor(X.values).unsqueeze(1).float().cuda()
...
def Compile_train(module, hyperparams,X,Y,data_amount=100000,p=0.85):
    x_train,x_test,y_train,y_test=sklearn.model_selection.train_test_split(X,Y,shuffle=True,train_size=p)    
...
def Hyper_parameter_GridSearch(
    ...
    for value in (grid):
        ...
        monte_carlo_test_acc, mean_acc_test,IC_acc_test = Hyperparameter_Test(supra_epochs=supra_epochs,hyperparameters=hyperparams,model_class=model_class,X=x,Y=y)
    ...
    return Optimum_parameter,Final_acc
    ...
    for param in (parameters):
        ...
        if param in grids:
            ...
            for value in grid_values:
                ...
                Opt_params,New_acc = Hyper_parameter_GridSearch(
                    best_parameters,
                    grid=grid_values,
                    parameter=param,
                ...
                Final_acc=torch.cat([Final_acc,New_acc],dim=1)
